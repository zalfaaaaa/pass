        ...
        <div class="card-body">
            <form action="upg-stu.php" method="post">
                ...
                <div class="row">
                    <div class="col mt-2 mb-3">
                        ...
                        <select name="idspp" class="form-select">
                            ...
                            foreach ($query as $query) :?>
                                <option value="<?php echo $query['idspp']?>"><?php echo $query['idspp']?> - <?= $query['year']?></option>
                            <?php endforeach ?>
                        </select>
                    </div>
                    <div class="col mt-2 mb-3">
                    <label for="form-label" class="fw-bold mb-1">Id Class</label>
                        <select name="idcls" class="form-select">
                            ...
                            foreach ($query as $query) :?>
                                <option value="<?php echo $query['idcls']?>"><?php echo $query['idcls']?> - <?= $query['clsname']?> - <?= $query['skillcom']?></option>
                            <?php endforeach ?>
                        </select>
                    </div>
                </div>
                ...
                <div class="mt-3 text-center d-flex">
                    <a href="student.php" class="btn btn text-center text-light fw-bold shadow" onclick="return confirm('Are You Sure Want To Cancel?')" style="width:150px;background-color:#ba0000;border-radius:12px">Cancel</a>&emsp;
                    <button type="submit" onclick="return confirm('Are You Sure You Want To Update This?')" class="btn btn text-center fw-bold shadow" style="width:150px;background-color:#8DCBE6;border-radius:12px">Save</button>
                </div>
            </form>
        </div>
      </div>
